#!/bin/bash
#USB Rubber Ducky script to raspberry pi HID converter, modified by me
#This layout is for QWERTZ Czech keyboard for Windows specifically
defdelay=0
kb="/dev/hidg0 keyboard"
last_cmd=""
last_string=""
line_num=0
#Completely different layout, original was for US keyboard
function convert() 
{
		local kbcode=""
	    if [ "$1" == " " ]
...
while IFS='' read -r line || [[ -n "$line" ]]; do
	...
	then
		...
		for ((  i=0; i<${#info}; i++  )); do
			...
			if [ "$kbcode" != "" ]
			then
				echo "$kbcode" | /home/kali/Rpi_BadUSB/hid-gadget-test $kb > /dev/null
			fi
		done
	elif [ "$cmd" == "ENTER" ] 
	then
		last_cmd="enter"
		echo "$last_cmd" | /home/kali/Rpi_BadUSB/hid-gadget-test $kb > /dev/null
	
	elif [ "$cmd" == "DELAY" ] 
	then
		last_cmd="UNS"
		((info = info*1000))
		/home/kali/Rpi_BadUSB/usleep  $info
	...
	then
		...
		then
			...
			for ((  i=0; i<$info; i++  )); do
				...
				/home/kali/Rpi_BadUSB/usleep $defdelay
			done
		fi
